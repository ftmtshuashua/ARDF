 ...
 *      isConnected()           :返回网络是否连接
 *      isAvailableByPing()     :返回网络是否可用ping(检查网络连通性)
 *      getMobileDataEnabled()  :返回是否启用了移动数据
 *      setMobileDataEnabled()  :设置启用了移动数据
 *      isMobileData()          :返回是否使用移动数据
 *      is4G()                  :返回是否使用4g
 *      getWifiEnabled()        :返回是否启用了wifi
 *      setWifiEnabled()        :设置启用了wifi
 *      isWifiConnected()       :返回wifi是否连接
 *      isWifiAvailable()       :检查wifi连通性
 *      getNetworkOperatorName():返回网络的名称
